...
const OfferList: React.FC = () => {
    ...
    useEffect(() => {
        const fetchOffers = async () => {
            try {
                const response = await fetch('http://localhost:9000/bns/offer/retrieve-all-offers');
                if (!response.ok) {
        ...
        <div className="container mx-auto my-8">
            ...
            <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                {offers.map((offer, index) => (
                    ...
                    >
                        <div className="mb-4">
                            <Image src="https://images.freeimages.com/images/premium/previews/5905/59051846-shop-building-display-window-sale-drawing.jpg" alt="Offer Image" className="rounded-md" />
                        </div>
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg font-semibold">{offer.namePer}</h2>
                            <Button color="primary" size="small" onClick={() => handleViewDetails(offer)}>
                                View Details
                            </Button>
                        </div>
                        <p className="text-gray-600 mb-2">Shop Name: {offer.shopName}</p>
                        <p className="text-gray-600 mb-2">Business Type: {offer.businessType}</p>
